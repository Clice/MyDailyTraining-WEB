...
$listarChamados = "SELECT alunos.nomeAluno, chamados.horarioChamado FROM chamados INNER JOIN usuarios ON chamados.idUsuario = usuarios.idUsuario INNER JOIN alunos ON chamados.idAluno = alunos.idAluno WHERE chamados.idUsuario = ? AND chamados.idAcademia = ? AND chamados.dataChamado = ? ORDER BY chamados.horarioChamado ASC";
...
if ($rsListarChamados) {
    if ($rsListarChamados->rowCount() > 0) {
        ...
        /* while ($chamado = $rsListarChamados->fetch(PDO::FETCH_ASSOC)) {
          $chamados[] = $chamado;
          }
